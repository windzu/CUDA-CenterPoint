...
- 本工程 fork 自 NVIDIA 的仓库：[NVIDIA Lidar_AI_Solution](https://github.com/NVIDIA-AI-IOT/Lidar_AI_Solution.git) 中的 `CUDA-CenterPoint`。
- 原工程已有较长时间未维护，尤其在兼容新版 `spconv` 和对 TensorRT 10.x 的支持方面存在不足。为了解决这些问题，本工程完成了对新版 `spconv` 的适配与支持；该 `spconv` 源自 upstream 的 [3DSparseConvolution](https://github.com/NVIDIA-AI-IOT/Lidar_AI_Solution/tree/master/libraries/3DSparseConvolution)，并已迁移并封装到本仓库的 `third_party/spconv` 目录下。我们计划在后续版本中定期与 upstream 的 3DSparseConvolution 库保持同步更新。
- 本工程还提供了一个 devcontainer 开发环境，以便更方便、快速地复现结果和进行开发（请参见仓库中的 devcontainer 配置）。
